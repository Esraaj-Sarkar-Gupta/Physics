...
try:
    os.makedirs('Logs') # Make log folder
except:
    pass # Log folder already exists 
...
def Electro(k , q1 , q2 , r1, r2): # Coloumb's Law of Electrostatics 
    ...
    F_head = (1)* displacement/ disp_mag
...
System_data_helper_1 = [] # Holds velocities if each object in the system for each instance of time
System_data_helper_2 = [] # Holds accelerations of each object in the system for each instance of time

for _ in t: # Add rows ro System_data and System_data_helper required for each time instance
    System_data.append([])
    System_data_helper_1.append([])
    System_data_helper_2.append([])
    
for i in range(len(System) - s):
    System_data[0].append(System[i]['position']) # Append initial positions into System_data
    System_data_helper_1[0].append(System[i]['velocity']) # Append initial values of veclocity in System_data_helper_1
    System_data_helper_2[0].append(np.array([0.0 , 0.0])) # Append initial zeros into System_data_helper_2
...
for j in range(len(t)): # Computes System for each instance of time
    ...
    for n in range(len(System)): # Computes interactions on each object by the rest of the objects in the system
        ...
        System_data_helper_1[j].append(V) # Append the updates values of velocity to the System_data_helper for current time instance
        System_data_helper_2[j].append(a) # Append the updated acceleration to the System_data_helper for current time instance
...
for i in range(len(t)):
    ...
    for j in range(len(System_data[i])): # Pull data from System_data and System_data_helper to plot
        #print(j)
        x = System_data[i][j][0] #Pull coordinates from system data
        y = System_data[i][j][1]
        
        v_x = System_data_helper_1[i][j][0] # Pull velocity vector components from System_data_helper_1
        v_y = System_data_helper_1[i][j][1]
        
        a_x = System_data_helper_2[i][j][0] # Pull acceleration vector components from System_data_helper_2
        a_y = System_data_helper_2[i][j][1]
        
        Ex_v = [x , (v_x + x) * alpha] # Velocity vector to be plotted treating the position of the object as local origin
        Why_v = [y , (v_y + x) * alpha]
        
        
        Ex_a = [x , (a_x + x) * alpha] # Acceleration vector tobe plotted treating the position of the object as local origin
        Why_a = [y , (a_y + y) * alpha]
        ...
        plt.plot(Ex_a, Why_a , color = 'red') # Plot the scaled acceleration vector
        plt.plot(Ex_v , Why_v , color = 'blue') # Plot the scaled velocity vectore
    try:
